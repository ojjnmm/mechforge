...
class MechForgeConfig:
    ...
    # 内部字段
    _config_file: Optional[Path] = field(init=False, repr=False)

    def __post_init__(self):
        self._config_file = _get_default_config_file()

    @classmethod
    def from_file(cls, config_path: Optional[str] = None) -> "MechForgeConfig":
        """从文件加载配置"""
        if config_path:
    ...
    def get_provider_config(self, provider: Optional[str] = None) -> Any:
        """获取指定 provider 的配置"""
...
_default_config: Optional[MechForgeConfig] = None


def get_config(config_path: Optional[str] = None, force_reload: bool = False) -> MechForgeConfig:
    """获取配置（单例）"""
...
def save_config(config: MechForgeConfig, config_path: Optional[str] = None):
    """保存配置到文件"""
