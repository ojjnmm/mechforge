...
class CalculiXEngine:
    ...
    def generate_inp(
        ...
        inp_content = self._generate_inp_content(
            analysis_type, boundary_conditions, material
        )
    ...
    def solve_via_api(
        ...
        try:
            ...
            if result.get("success"):
                ...
                return SolveResult(
                    success=False,
                    error=result.get("error", "API 返回未知错误")
                )

        except urllib.error.URLError as e:
            return SolveResult(
                success=False,
                error=f"API 连接失败: {e.reason}"
            )
        except urllib.error.HTTPError as e:
            return SolveResult(
                success=False,
                error=f"API HTTP 错误: {e.code} {e.reason}"
            )
        except Exception as e:
            return SolveResult(
                success=False,
