        ...
        try:
            ...
            if displacement_values is None:
                ...
                displacement_values = np.array([
                    [random.uniform(-0.01, 0.01) for _ in range(3)]
                    for _ in range(n_points)
                ])
            
            # 创建变形网格
            warped = self._mesh.warp_by_vector(
                "displacement",
                factor=warp_factor
            ) if "displacement" in self._mesh.array_names else self._mesh
...
def visualize_result(
    ...
    return VisualizationResult(
        success=False,
        error="PyVista 不可用，请使用 ASCII 模式"
    )


# 单例
_viz_instance: Optional[PyVistaEngine] = None
